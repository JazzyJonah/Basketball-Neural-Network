...
class SeasonToDateFeatureStore:
    def __init__(self, games_df: pd.DataFrame, cfg: Optional[FeatureConfig] = None):
        ...
        self._as_of_feature_lookup: Dict[Tuple[int, int, pd.Timestamp], np.ndarray] = {}
        self._team_history_lookup: Dict[Tuple[int, int], pd.DataFrame] = {
            (int(season), int(team_id)): group.sort_values(
                ["date", "opp_id"]
            ).reset_index(drop=True)
            for (season, team_id), group in self.team_games.groupby(
    ...
    def build_or_load_feature_cache(self, rebuild: bool = False) -> pd.DataFrame:
        ...
        for name in self.feature_names:
            feature_table[name] = (
                pd.to_numeric(feature_table[name], errors="coerce")
                .fillna(0.0)
                .astype(np.float32)
            )
    ...
    def get_team_features_as_of(
        self, team_id: int, date: pd.Timestamp, season: int
    ) -> Dict[str, float]:
        """
        Build season-to-date features for an arbitrary query date.

        Unlike get_team_features(), this does not require the query date to be an
        existing game date in the historical feature cache. Only completed games
        with dates strictly earlier than the query date are used. This is the
        method used for true future-game predictions.
        """
        vector = self.get_team_feature_vector_as_of(team_id, date, season)
        return {name: float(vector[i]) for i, name in enumerate(self.feature_names)}

    def get_team_feature_vector_as_of(
        self, team_id: int, date: pd.Timestamp, season: int
    ) -> np.ndarray:
        query_date = pd.Timestamp(date).normalize()
        key = (int(season), int(team_id), query_date)

        cached = self._as_of_feature_lookup.get(key)
        if cached is not None:
            return cached.copy()

        team_history = self._team_history_lookup.get((int(season), int(team_id)))
        if team_history is None or team_history.empty:
            vector = np.zeros(len(self.feature_names), dtype=np.float32)
        else:
            # Scheduled-but-unplayed games never enter team_games, because this
            # store is built only from rows with complete box-score data. The
            # date filter additionally prevents any game on/after the target date
            # from leaking into the prediction.
            history = team_history.loc[team_history["date"] < query_date]
            features = summarize_history(history, query_date, self.cfg)
            vector = np.array(
                [features[name] for name in self.feature_names], dtype=np.float32
            )

        self._as_of_feature_lookup[key] = vector
        return vector.copy()
...
def build_matchup_feature_frame(
    ...
    for name in fill_bar:
        matchups[f"team1_{name}"] = (
            pd.to_numeric(matchups[f"team1_{name}"], errors="coerce")
            .fillna(0.0)
            .astype(np.float32)
        )
        matchups[f"team2_{name}"] = (
            pd.to_numeric(matchups[f"team2_{name}"], errors="coerce")
            .fillna(0.0)
            .astype(np.float32)
        )
    ...
    derived["home_indicator"] = (
        matchups["team1home"].astype(np.float32)
        - matchups["team2home"].astype(np.float32)
    ).astype(np.float32)
